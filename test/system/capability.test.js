...
test(
	...
	setupThenTransactionTest(
		(t, connection, txn) => {
			...
			txn.write(root, Buffer.from("root obj"), [
				o1.denyRead(),
				o1.denyWrite(),
				o1.denyRead().denyWrite()
			])
		},
		(t, connection, txn) => {
			const root = txn.root(testRootName)
			...
			t.deepEqual(newRefs, [])
			t.throws(
				() => txn.write(noWriteyRef, Buffer.from("this shouldn't work"), []),
				goshawkdb.errors.CapabilityDenied
			)

			t.throws(() => txn.read(noAnythingRef), goshawkdb.errors.CapabilityDenied)
			t.throws(
				() => txn.write(noAnythingRef, Buffer.from("this shouldn't work"), []),
				goshawkdb.errors.CapabilityDenied
			)
		}
	)
