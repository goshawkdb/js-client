...
test(
	"Nested transactions can provide values that their parents use.",
...
test(
	"Nested transactions can throw errors and not affect their parent.",
	transactionMustFail(TransactionRejectedError, async (t, connection, txn) => {
		let ref = null
		await txn
			.transact(async subTxn => {
...
test(
	"Nested transactions can create objects that their parents can use.",
	transactionTest(async (t, connection, txn) => {
...
test(
	"Nested transactions can retry.",
	setupThenTest(
		transactionTest((t, connection, txn) =>
			txn.write(txn.root(testRootName), Buffer.from([0]), [])
		),
		transactionTest((t, connection, txn) => {
			t.plan(1)
			txn.transact(subTxn => {
